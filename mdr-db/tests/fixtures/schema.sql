...
--
-- The normalized replacement for md_contribution. Both are written by
-- mdr-process for now; see md-repo-app docs/creator-normalization.md.
-- Generated from md-repo-app migration 0275 via pg_dump, not hand-written.
--
...
-- The identity key. NULL-safe on purpose: a plain UNIQUE would treat NULLs as
-- distinct and enforce nothing, since most rows carry at least one.
CREATE UNIQUE INDEX uniq_creator_identity ON public.md_creator USING btree
    (lower(COALESCE(name, ''::text)),
     COALESCE(orcid, ''::character varying),
     lower((COALESCE(email, ''::character varying))::text),
     lower(COALESCE(institution, ''::text)));
...
CREATE INDEX md_simulation_creator_creator_id_12d0500b
    ON public.md_simulation_creator USING btree (creator_id);

ALTER TABLE ONLY public.md_simulation_creator
    ADD CONSTRAINT md_simulation_creato_simulation_id_372e91d4_fk_md_simula
    FOREIGN KEY (simulation_id) REFERENCES public.md_simulation(id) DEFERRABLE INITIALLY DEFERRED;
